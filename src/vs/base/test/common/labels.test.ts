...
suite('Labels', () => {
	...
	test('template', () => {
		...
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: '', rootName: '', appName: 'CEC-IDE', separator: { label: ' - ' } }), 'CEC-IDE');
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: 'Untitled-1', rootName: '', appName: 'CEC-IDE', separator: { label: ' - ' } }), 'Untitled-1 - CEC-IDE');
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: '', rootName: 'monaco', appName: 'CEC-IDE', separator: { label: ' - ' } }), 'monaco - CEC-IDE');
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: 'somefile.txt', rootName: 'monaco', appName: 'CEC-IDE', separator: { label: ' - ' } }), 'somefile.txt - monaco - CEC-IDE');
		assert.strictEqual(labels.template(t, { dirty: '* ', activeEditorShort: 'somefile.txt', rootName: 'monaco', appName: 'CEC-IDE', separator: { label: ' - ' } }), '* somefile.txt - monaco - CEC-IDE');

		// real world example (other)
		t = '${dirty}${activeEditorShort}${separator}${rootNameShort}${separator}${appName}';
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: '', rootName: 'monaco (Workspace)', rootNameShort: 'monaco', appName: 'CEC-IDE', separator: { label: ' - ' } }), 'monaco - CEC-IDE');
	});
