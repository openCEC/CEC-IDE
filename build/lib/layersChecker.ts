...
function checkFile(program: ts.Program, sourceFile: ts.SourceFile, rule: IRule) {
	...
	function checkNode(node: ts.Node): void {
		...
		if (rule.disallowedTypes?.some(disallowed => disallowed === text)) {
			...
			console.log(`[build/lib/layersChecker.ts]: Reference to type '${text}' violates layer '${rule.target}' (${sourceFile.fileName} (${line + 1},${character + 1}). Learn more about our source code organization at https://github.com/opencec/CEC-IDE/wiki/Source-Code-Organization.`);
							...
							if (rule.disallowedDefinitions) {
								for (const disallowedDefinition of rule.disallowedDefinitions) {
									if (definitionFileName.indexOf(disallowedDefinition) >= 0) {
										...
										console.log(`[build/lib/layersChecker.ts]: Reference to symbol '${text}' from '${disallowedDefinition}' violates layer '${rule.target}' (${sourceFile.fileName} (${line + 1},${character + 1}) Learn more about our source code organization at https://github.com/opencec/CEC-IDE/wiki/Source-Code-Organization.`);
